...
AuthorizeHandler.prototype.handle = function(request, response) {
  ...
  return Promise.all(fns)
	...
	.then(function() {
          // scope = this.getScope(request); // puck
          scope = validateScope(user, client, this.getScope(request)); // puck
          if(typeof scope === 'boolean' && !validScope) {
...
AuthorizeHandler.prototype.getScope = function(request) {
  ...
  if (!is.nqschar(scope)) {
    // throw new InvalidScopeError('Invalid parameter: `scope`');
    return undefined; // RFC 6749 scope is OPTIONAL
  }
