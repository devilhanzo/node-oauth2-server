...
AuthorizeHandler.prototype.handle = function(request, response) {
  ...
  var fns = [
    ...
    this.getPassport(request)
  ];

  return Promise.all(fns)
    .bind(this)
    .spread(function(expiresAt, client, user, passport) {
	...
	.then(function() {
	  ...
	  return this.generateAuthorizationCode(client, user, scope);
        ...
        .then(function(authorizationCode) {
          ...
          return this.saveAuthorizationCode(authorizationCode, expiresAt, scope, client, uri, user, passport);
        })
        .then(function(code) {
 ...
 */
AuthorizeHandler.prototype.validateAuthorizeScope = function(client, scope) {
  if (this.model.validateAuthorizeScope) {
    return promisify(this.model.validateAuthorizeScope, 3).call(this.model, client, scope)
      .then(function (scope) {
        if (typeof scope === 'boolean' && !scope) {
 ...
 * Get User passport
 */
AuthorizeHandler.prototype.getPassport = function(request) {
  if (!request.session.passport.user) {
    throw new InvalidRequestError('Missing parameter: `User Passport`');
  }
...
AuthorizeHandler.prototype.saveAuthorizationCode = function(authorizationCode, expiresAt, scope, client, redirectUri, user, passport) {
  var code = {
    authorizationCode: authorizationCode,
    expiresAt: expiresAt,
    redirectUri: redirectUri,
    scope: this.validateAuthorizeScope(client, scope)
    ,passport: passport
  };
