...
ClientCredentialsGrantType.prototype.saveToken = function(user, client, scope) {
  var validScope = this.validateScope(user, client, scope);
  if(typeof validScope === 'boolean' && !validScope) {
    throw new InvalidScopeError('Invalid scope: provide scope not exist')
  }
  var fns = [
    validScope,
